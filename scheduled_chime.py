...
class ChimeApp:
    ...
    def __init__(self, root):
        ...
        # --- タイトルとバージョン表記 ---
        APP_VERSION = "v1.2"
        self.root.title(f"定刻チャイム   {APP_VERSION}")
        self.root.geometry("310x300")

        # --- ウィンドウ左上のアイコン設定 ---
        ico_path = resource_path("chime.ico")
    ...
    def select_file(self):
        ...
        if file_path:
            ...
            # 画面にはファイル名のみを表示（os.path.basename を使用）
            filename_only = os.path.basename(file_path)
            self.file_entry.delete(0, tk.END)
            self.file_entry.insert(0, filename_only)
    ...
    def start_timer(self):
        # 内部で保持しているフルパスを使用
        wav_path = self.selected_wav_path.strip()

        # 音源ファイル未選択チェック
        if not wav_path:
            messagebox.showwarning(
                "警告", "音源ファイル（WAV）を選択してください。"
            )
            return

        # 指定されたファイルが存在しない場合のチェック
